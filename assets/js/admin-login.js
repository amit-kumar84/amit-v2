(function(){'use strict';
  ...
  ready(function(){
    ...
    // add floating shapes to left panel
    if(left){
    ...
    if(pwd){
      ...
      const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'pwd-toggle'; btn.setAttribute('aria-label','Show password'); btn.innerHTML = '<i class="fas fa-eye"></i>';
      wrap.appendChild(btn);
      btn.addEventListener('click', function(){ if(pwd.type==='password'){ pwd.type='text'; this.innerHTML = '<i class="fas fa-eye-slash"></i>'; this.setAttribute('aria-label','Hide password'); } else { pwd.type='password'; this.innerHTML = '<i class="fas fa-eye"></i>'; this.setAttribute('aria-label','Show password'); } });
    }

    // enhance submit: show spinner, disable
    const submit = form && form.querySelector('button[type="submit"], .btn-navy');
    if(submit && form){
      form.addEventListener('submit', function(e){
        try{
          submit.disabled = true; submit.classList.add('submitting');
          const spinner = document.createElement('span'); spinner.className = 'spinner-border spinner-border-sm ms-2'; spinner.setAttribute('role','status'); spinner.setAttribute('aria-hidden','true'); submit.appendChild(spinner);
        }catch(err){/* ignore */}
      });
    }

    // small keyboard focus effect for inputs
